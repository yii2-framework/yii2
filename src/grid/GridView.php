...
class GridView extends BaseListView
{
     ...
     * HTML attributes for all body rows, or an anonymous function that returns an array of the HTML attributes.
     * The anonymous function will be called once for every data model returned by [[dataProvider]]. It should have the
     * following signature:
     *
     * ```
     ...
     *
     * It should have the similar signature as [[rowOptions]]. The return result of the function will be rendered
     * directly.
     */
    public $beforeRow;
    /**
     ...
     *
     * This property is used to render cells that have no defined content, for example, empty footer or filter cells.
     *
     * Note that this is not used by the [[DataColumn]] if a data item is `null`. In that case the
     * [[\yii\i18n\Formatter::nullDisplay|nullDisplay]] property of the [[formatter]] will be used to indicate an empty
     * data value.
     */
    public $emptyCell = '&nbsp;';
    /**
     * @var \yii\base\Model|null The model that keeps the user-entered filter data. When this property is set, the grid
    ...
    public $filterModel;
    /**
     * @var string|array|null The URL for returning the filtering result. [[\yii\helpers\Url::to()]] will be called to
     ...
     *
     * If this is a Closure, it gets the widget ID and the filter row ID as parameters and is expected to return the
     * selector.
     *
     * By default, this selector is added to the default selector '#$id input, #$id select' (where $id is the filter
     * row ID).
     * If [[overrideFilterSelector]] is set, this selector is used instead of the default selector.
     */
    public $filterSelector;
    /**
     * @var bool If set, [[filterSelector]] overrides the default selector of '#$id input, #$id select' (where $id is
     * the filter row ID) rather than being added to it.
     */
