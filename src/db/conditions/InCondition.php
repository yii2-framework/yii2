...
/**
 * Represents an `IN` condition for use in query building.
 *
...
class InCondition implements ConditionInterface
{
     ...
     * @param array|int|string|ExpressionInterface|Traversable $values an array of values that {@see $column} value
     * should be among. If it is an empty array the generated expression will be a `false` value if {@see $operator} is
     * `IN` and empty if operator is `NOT IN`.
     */
    ...
    /**
     * Returns the operator to use (for example, `IN` or `NOT IN`).
     */
